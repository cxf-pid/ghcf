...
if args.dataset=='amazon-book':
    ...
    BATCH_SIZE = args.batch_size//2  #256/2
...
def test(sess, model, users_to_test, drop_flag=False, batch_test_flag=False,train_set_flag=0):
    ...
    pool = multiprocessing.Pool(cores) #线程池个数

    u_batch_size = BATCH_SIZE # 128
    i_batch_size = BATCH_SIZE

    test_users = users_to_test #测试集用户
    n_test_users = len(test_users) #测试用户个数
    n_user_batchs = n_test_users // u_batch_size + 1 #批数
